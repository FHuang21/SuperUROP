...
class MultiClassDataset(Dataset):
    def __init__(self, root, transform=None):
        ...
        for image_name in tqdm(self.image_names): # seems inefficient to do this loop every time. is there a way to avoid this???
            # idea: use pickle to store list of image_paths and corresponding labels
            current_path = self.root + 'control/' + image_name + '.jpg'
            current_label = 0
            if is_image_in_folder(image_name + '.jpg', self.root + '/tca') and is_image_in_folder(image_name + '.jpg', self.root + '/ntca'):
    ...
    def __getitem__(self, index):
        ...
        # get sleep stage info to cut non-sleep at end of array
