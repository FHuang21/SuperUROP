...
import math
import numpy as np
from .Flowformer_layers import Encoder, EncoderLayer
...
class FlowformerClassiregressor(nn.Module):
    ...
    def forward_with_visualization(self, x_enc):
        inp = x_enc.permute(1, 0, 2)
        inp = self.project_inp(inp) * math.sqrt(
            self.d_model)  # [seq_length, batch_size, d_model] project input vectors to d_model dimensional space
        inp = self.pos_enc(inp)  # add positional encoding
        ...
        enc_out, att = self.encoder.forward_with_visualization(inp)
        enc_out = enc_out.permute(1, 0, 2)
        output = self.act(enc_out)  # the output transformer encoder/decoder embeddings don't include non-linearity
        output = output.permute(1, 0, 2)  # (batch_size, seq_length, d_model)
        output = self.dropout(output)
...
if __name__ == '__main__':
    model = FlowformerClassiregressor()
